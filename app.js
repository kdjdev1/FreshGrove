...
const firebaseConfig = {
  apiKey: "AIzaSyC-9rDFuc8PkuLi2MwLp6ppYvPr1rK58XU",
  authDomain: "freshgrove.firebaseapp.com",
  projectId: "freshgrove",
  storageBucket: "freshgrove.firebasestorage.app",
  messagingSenderId: "871661487138",
  appId: "1:871661487138:web:11b035d485900ff1c8367b",
  measurementId: "G-WZ7Q8VR1DK"
};
...
// ── Main Admin (hardcoded, never stored in Firebase) ──
const MA = {
  username: 'oshan',
  password: 'oshan5555',
  name: 'P.A.O. Angel',
...
// ══════════════════════════════════════════
//  SEED DEFAULT PRODUCTS (runs once if empty)
// ══════════════════════════════════════════
async function seed() {
  ...
  const defaults = [
    ...
    { name:"Sweet Mango", category:"fruit", price:480, unit:"per kg", desc:"Juicy Willard mangoes picked at peak ripeness. Naturally sweet with golden-orange flesh.", emoji:"🥭", available:true, img:"https://images.unsplash.com/photo-1601493700631-2b16ec4b4716?w=600&q=80", addedBy:"oshan", addedByName:"P.A.O. Angel" },
    { name:"King Coconut", category:"fruit", price:120, unit:"each", desc:"Fresh thambili from Kelaniya farms. Packed with natural electrolytes.", emoji:"🥥", available:true, img:"https://images.unsplash.com/photo-1560637902-e019f5a8c5e7?w=600&q=80", addedBy:"oshan", addedByName:"P.A.O. Angel" },
    ...
    { name:"Red Tomatoes", category:"vegetable", price:140, unit:"per 500g", desc:"Plump, vine-ripened tomatoes from Jaffna's fertile red soil. Perfect for curries.", emoji:"🍅", available:true, img:"https://images.unsplash.com/photo-1546094096-0df4bcaaa337?w=600&q=80", addedBy:"oshan", addedByName:"P.A.O. Angel" },
...
function startListeners() {
  ...
  onSnapshot(collection(db, 'admins'), snap => {
    ...
    if (CU?.role === 'main') { renderAdmins(); updateStats(); }
  });
}
...
function renderPublic(f) {
  ...
  if (!list.length) {
    grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1;text-align:center;padding:3rem;color:var(--muted)"><div style="font-size:3rem;margin-bottom:1rem">🌿</div>No products yet!</div>';
    return;
  }
  grid.innerHTML = list.map(p => `
    <div class="product-card">
      ...
      <div class="product-body">
        ...
        <div style="font-size:0.72rem;color:var(--muted);margin-top:0.4rem;">By: ${p.addedByName || p.addedBy || 'FreshGrove'}</div>
      </div>
    </div>`).join('');
}
...
// ══════════════════════════════════════════
const AI_REPLIES = {
  papaya:    "🍈 Ripe Red Papaya is at peak freshness! Loaded with Vitamin C and papain enzymes for digestion. Harvested from Kurunegala this morning. Try it chilled or in a smoothie!",
  ...
  recipe:    "🍽️ Tell me which dish you're cooking and I'll list the exact ingredients you need from FreshGrove!",
  fresh:     "✅ All FreshGrove products are harvested within 24 hours of delivery. We work with 50+ local farmers with strict freshness checks.",
...
window.sendChat = function() {
  ...
  msgs.innerHTML += `<div class="msg user">${text}</div><div class="msg typing">Grove AI is thinking...</div>`;
  input.value = '';
...
window.doLogin = async function() {
  ...
  // Check seller admins from Firebase
  const snap = await getDocs(collection(db, 'admins'));
  let found = null;
  snap.forEach(d => {
    const a = d.data();
    if (a.username === u && a.password === p) found = { id: d.id, ...a };
  });
...
function buildUI() {
  ...
  document.getElementById('d-nav').innerHTML = `
    <div style="display:flex;align-items:center;gap:1rem;">
      <span class="dash-logo">Fresh<span>Grove</span></span>
  ...
  // Sidebar
  let sb = `
    <div class="dash-sidebar-section" style="font-size:0.68rem;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin:1rem 0 0.3rem 0.5rem;font-weight:700;">General</div>
    <button class="dash-tab" id="tb-overview" onclick="switchPanel('overview')">📊 <span>Overview</span></button>
    <div class="dash-sidebar-section" style="font-size:0.68rem;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin:1rem 0 0.3rem 0.5rem;font-weight:700;">Products</div>
...
function updateStats() {
  ...
  document.getElementById('d-welcome').textContent = `Welcome back, ${CU.name || CU.username}! Here's your live market summary.`;
  document.getElementById('d-summary').innerHTML = isMa
    ...
    : `→ You have ${mine.length} products in the market<br>→ ${mine.filter(p => p.available).length} available, ${mine.filter(p => !p.available).length} out of stock<br>→ Products are shown live on the public website`;
}
...
window.addProduct = async function() {
  ...
  try {
    await addDoc(collection(db, 'products'), {
      name, category: cat, price, unit, desc,
      available: avail, img, emoji: '🌿',
      addedBy: CU.username,
      addedByName: CU.name || CU.username,
      createdAt: serverTimestamp()
    });
    showToast(`✅ "${name}" added to market!`);
    clearAddForm();
    switchPanel('manage');
  } catch (e) { showToast('❌ Firebase error: ' + e.message, 'err'); }
};
...
window.saveEdit = async function(id) {
  const updates = {
    name:      document.getElementById('e-n').value.trim(),
    category:  document.getElementById('e-c').value,
    price:     parseInt(document.getElementById('e-p').value),
    unit:      document.getElementById('e-u').value.trim(),
    desc:      document.getElementById('e-d').value.trim(),
    available: document.getElementById('e-s').value === 'true'
  };
...
window.createAdmin = async function() {
  ...
  if (ADMINS.find(a => a.username === username) || username === MA.username) {
    showToast('❌ Username already taken', 'err'); return;
  }

  try {
    await addDoc(collection(db, 'admins'), {
      name, area, username, password, contact,
      role: 'seller',
      createdAt: serverTimestamp(),
      createdBy: 'oshan'
    });
    showToast(`✅ Seller admin "${name}" created!`);
    ['na-name', 'na-area', 'na-user', 'na-pass', 'na-contact'].forEach(id => document.getElementById(id).value = '');
  } catch (e) { showToast('❌ ' + e.message, 'err'); }
};
...
function renderAdmins() {
  ...
  if (!ADMINS.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--muted)">No seller admins yet. Create one above!</td></tr>';
    return;
  }
...
window.delAdmin = async function(id, name, uname) {
  if (CU?.role !== 'main') return;
  ...
  try {
    const theirProds = PRODS.filter(p => p.addedBy === uname);
    for (const p of theirProds) await deleteDoc(doc(db, 'products', p.id));
    ...
    <div class="profile-card">
      <div class="profile-photo-section">
        ...
        <div class="profile-name-block">
          ...
          <div class="role">${isMa ? '⭐ Main Administrator — FreshGrove' : '🌿 Seller Admin'}</div>
        </div>
      </div>
      <div class="profile-fields">
        ...
        ${CU.area    ? `<div class="profile-field"><div class="pf-icon">📍</div><div><div class="pf-label">Area</div><div class="pf-value">${CU.area}</div></div></div>` : ''}
...
window.addEventListener('load', async () => {
  try { await seed(); } catch (e) { console.log('Seed skipped:', e.message); }
  startListeners();
  setTimeout(() => {
